    ...
    */
    ...
    if(rotation_centralized){
      ...
      if(stop && iter != 0){ // Maybe the change is wrong at zero iteration
	    std::cout << "[optimizeRotation] Change is small enough with iter:" << iter << std::endl;
        break;
      }
#endif
    }
...
void optimizePose(std::vector< boost::shared_ptr<DistributedMapper> >& dist_mappers,
  ...
  for(size_t iter=0; iter < max_iter; iter++){
    ...
    for(size_t robot: ordering){   // Iterate each optimizer once
      ...
      if(debug) std::cout << "[optimizePoses] Queried neighbors"  << std::endl;
    ...
    if(pose_centralized){
      ...
      if(stop && iter!=0)
	  {
		std::cout << "[optimizePoses] Change is small enough with iter:"<< iter << std::endl;
		break;
	  }
#endif
    }
...
distributedOptimizer(std::vector< boost::shared_ptr<DistributedMapper> >& dist_mappers,
  ...
  if(debug)
    std::cout << "Starting Optimizer " << dist_mappers.size() << " " << nr_robots << std::endl;
